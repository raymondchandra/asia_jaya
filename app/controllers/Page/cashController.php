		...
		if($filtered == '0')
		{
			...
			if($dataAll != null){
				foreach($dataAll as $allData){
					$datas[] = (object)array('type'=>$allData->type, 'transaction_id'=>$allData->transaction_id, 'in_amount'=>$allData->in_amount, 'out_amount'=>$allData->out_amount, 'total'=>$allData->total, 'created_at'=>$allData->created_at);
				}
			}
